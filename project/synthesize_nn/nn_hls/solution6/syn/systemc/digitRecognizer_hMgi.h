...
#ifndef _digitRecognizer_hMgi_H_
#define _digitRecognizer_hMgi_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "digitRecognizer_hMgi_memcore.h"

SC_MODULE(digitRecognizer_hMgi) {
...
protected:
    ...
    // buffer instances
    digitRecognizer_hMgi_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(digitRecognizer_hMgi) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "digitRecognizer_hMgi_memcore_U_";
            name += szTmp;

            buffer[i] = new digitRecognizer_hMgi_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "digitRecognizer_hMgi.trace");
            // ports
